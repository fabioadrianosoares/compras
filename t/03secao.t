...
my $edicao = $t->tx->res->dom('html body ul li a')->first->attr('href');
...
$t->post_ok('/compras/secao/salvar' => form => $dados);
...
foreach my $nome ('A', 'B', 'C') {
	$t->post_ok('/compras/secao/salvar' => form => {id => '', nome => "Secao $nome", salvar => 'salvar'});
...
while (<$arquivo>) {
	if (/:: (.+)/) {
		$t->post_ok('/compras/secao/salvar' => form => {id => '', nome => $1, salvar => 'salvar'});
...
$t->post_ok('/compras/secao/salvar' => form => {id => 99999, nome => 'Dummy', salvar => 'salvar'});
$t->status_is(200)->text_is('html body h2' => 'Seção não encontrada');

$t->post_ok('/compras/secao/salvar' => form => {id => 99999, salvar => 'salvar'});
