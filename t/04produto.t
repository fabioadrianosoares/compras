...
sub ParseComboSecoes {
	...
	my $dom = shift;
	my @retorno = ();
	$dom->at('html > body > form > select[name="secao"]')->find('option')->each(sub {
			push @retorno, {id => $_->attrs('value'),
				nome => $_->text};
		});
	return @retorno;
}
...
sub ParseListaProdutos {
	my $dom = shift;
	my @retorno = ();
	$dom->find('html > body > ul > li')->each(sub {
		my $link = $_->at('a[href^="/compras/secao/editar/"]');
		if ($link && $link->attrs('href') =~ m|/secao/editar/(\d+)|) {
			my $secao = {id => $1,
				nome => $link->text,
				link => $link->attrs('href'),
				produtos => []};
			$_->find('ul > li > a')->each(sub {
				if ($_->attrs('href') =~ m|/produto/editar/(\d+)|) {
...
$t->post_form_ok('/compras/produto/salvar' => {id => '', nome => 'Dummy', secao => $id_secao_dummy, salvar => 'salvar'});
...
my $dados = {id => $t->tx->res->dom('form input[name="id"]')->first->attrs('value'),
	secao => $t->tx->res->dom('form select[name="secao"]')->first->at('option[selected]')->attrs('value'),
...
$t->post_form_ok('/compras/produto/salvar' => $dados);
...
$t->post_form_ok('/compras/secao/salvar' => {id => '', nome => 'Dummy Nova', salvar => 'salvar'});
...
while (<$arquivo>) {
	if (/:: (.+)/) {
		...
		if ($secao_atual) {
			$t->post_form_ok('/compras/produto/salvar' => {id => '', secao => $secao_atual, nome => $1, salvar => 'salvar'});
...
$t->post_form_ok('/compras/produto/salvar' => {id => 99999, nome => 'Dummy', salvar => 'salvar'});
$t->status_is(200)->text_is('html body h2' => 'Produto não encontrado');

$t->post_form_ok('/compras/produto/salvar' => {id => 99999, salvar => 'salvar'});
