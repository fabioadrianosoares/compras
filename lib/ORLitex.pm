...
sub insert {
	my \$self = shift;
	my \$dbh  = $pkg->dbh;
	return if (defined(\&before_insert) && !\&before_insert(\$self));
	\$dbh->do(
		'$t->{sql_insert}',
		{},
$t->{pl_insert}
...
$t->{pl_fill}
	\&after_insert(\$self) if (defined(\&after_insert));
	return \$self;
}
...
sub update {
	my \$self = shift;
	my \%set  = \@_;
	return if (defined(\&before_update) && !\&before_update(\$self, \%set));
	my \$rows = $pkg->do(
		'update $t->{qname} set ' .
		join( ', ', map { "\\"\$_\\" = ?" } keys \%set ) .
		' where "rowid" = ?',
	...
	$set
	\&after_update(\$self, \%set) if (defined(\&after_update));
	return 1;
}

sub delete {
	return if (defined(\&before_delete) && !\&before_delete(\@_));
	return $pkg->do(
...
sub delete_where {
	return if (defined(\&before_delete_where) && !\&before_delete_where(\@_));
